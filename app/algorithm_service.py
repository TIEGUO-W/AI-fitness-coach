...
class PoseAlgorithmEngine:
    ...
    def _recognize_stage(
        ...
        if exercise == "squat":
            return self._recognize_squat_stage(angles, cleaning, state)
        if exercise == "push_up":
    ...
    def _recognize_squat_stage(self, angles, cleaning, state):
        """Dual-angle squat: knee+hip must bend, 10-frame confirm, auto-calibrate"""
        knee=angles.knee_angle; hip=angles.hip_angle
        if knee is None or hip is None: return "unknown",None
        ex="squat"
        if not hasattr(self,"_cal"): self._cal={};self._ctr={};self._pend={};self._pk={};self._ph={}
        self._cal.setdefault(ex,[]); self._ctr.setdefault(ex,0); self._pend.setdefault(ex,None); self._pk.setdefault(ex,None); self._ph.setdefault(ex,None)
        if len(self._cal[ex])<8: self._cal[ex].append(knee); return "standing",knee
        if not hasattr(self,"_done_cal"): self._done_cal={}
        if not self._done_cal.get(ex): self._baseline=max(self._cal[ex]); self._done_cal[ex]=True
        bl=self._baseline; desc=bl*0.85; bot=bl*0.65; asc=bl*0.72; stand=bl*0.90
        prev_k=self._pk[ex]; prev_h=self._ph[ex]
        d="none"
        if prev_k is not None:
            if knee<prev_k-3: d="down"
            elif knee>prev_k+3: d="up"
        hm=prev_h is not None and abs(hip-prev_h)>2
        self._pk[ex]=knee; self._ph[ex]=hip
        ps=state.get("previous_stage","unknown"); ns=self._pend[ex] or ps
        if ps in ("standing","unknown"):
            if knee<desc and hm and d=="down": ns="descending"
        elif ps=="descending":
            if knee<bot: ns="bottom"
            elif knee>stand: ns="standing"
        elif ps=="bottom":
            if knee>asc and d=="up": ns="ascending"
        elif ps=="ascending":
            if knee>stand and d=="up": ns="standing"
        if ns!=self._pend[ex]: self._pend[ex]=ns; self._ctr[ex]=1
        else: self._ctr[ex]+=1
        if self._ctr[ex]>=10:
            if ns!=ps: state["previous_stage"]=ns
            self._ctr[ex]=0; self._pend[ex]=None
        return state.get("previous_stage",ns),knee
